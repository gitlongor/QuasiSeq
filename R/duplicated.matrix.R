...
{
	if (!is.matrix(x) || !(is.numeric(x) || is.logical(x) || is.character(x) ) || !identical(incomparables, FALSE) || (MARGIN[1L]!=1L && MARGIN[1L]!=2L) || length(MARGIN)!=1L )
...
{
	if (!is.matrix(x) || !(is.numeric(x) || is.logical(x) || is.character(x) ) || !identical(incomparables, FALSE) || (MARGIN!=1L && MARGIN!=2L) || length(MARGIN)!=1L )
