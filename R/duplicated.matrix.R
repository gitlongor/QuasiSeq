...
{
	if (!is.matrix(x) || !(is.numeric(x) || is.logical(x)) || !identical(incomparables, FALSE) || MARGIN!=1L)
		return(base::duplicated.matrix(x, incomparables, MARGIN, fromLast, ...))
	.Call(C_dupNumMat, x, as.logical(fromLast))
}
...
unique.matrix=function (x, incomparables = FALSE, MARGIN = 1, fromLast = FALSE, ...)
{
	...
	x[!.Call(C_dupNumMat, x, as.logical(fromLast)),,drop=FALSE]
}
