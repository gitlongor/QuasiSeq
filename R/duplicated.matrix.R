...
{
	if (!is.matrix(x) || !(is.numeric(x) || is.logical(x) || is.character(x) || is.complex(x) || is.raw(x) ) || !identical(incomparables, FALSE) || (MARGIN[1L]!=1L && MARGIN[1L]!=2L) || length(MARGIN)!=1L )
