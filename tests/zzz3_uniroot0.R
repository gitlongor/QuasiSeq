		...
		if(FFtestRslt){
			...
			tryCatch({ ## Uniroot algorithm
				...
				for(grpId in seq_len(rk)){
					...
					#if(	objFunc(expXbLower[grpId]) * objFunc(expXbUpper[grpId]) >=0 ) browser()
					tmpans = uniroot(objFunc, lower=startExpXb1[grpId]*bracketFactor, upper= startExpXb1[grpId]/bracketFactor, maxiter=control$maxit, tol=control$tol, extendInt='downX', check.conv=TRUE)
