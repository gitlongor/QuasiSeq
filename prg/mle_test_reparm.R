...
prg.name=paste('mle_test_reparm', Sys.Date(), sep='_')
...
source(file.path(prg.dir, "bnbRegMle_reparm.R"))
...
for(g in seq(G)) {
	tmp=bnbRegMle_reparm(counts.nb[g,], x=X,o=offs, start=c(fit$coefficients[g,], phis[g], taus[g]) , iter.max=5e3L)
	betahats[g,]=tmp$bet
